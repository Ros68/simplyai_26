...
// 🌟 FIX: Import Email Service at the top
import { sendQuestionnaireCompletionEmail } from "../services/emailService.js";
...
router.post("/save-questionnaire-completion", async (req, res) => {
  try {
    ...
    if (status === "draft") {
      ...
      // 🌟 FIX: TRIGGER COMPLETION EMAIL HERE (Without breaking your draft logic)
      try {
        const [settings] = await pool.query("SELECT send_completion_email FROM app_settings ORDER BY id DESC LIMIT 1");
        let sendCompletionEmail = false;

        if (settings.length > 0) {
          const val = settings[0].send_completion_email;
          sendCompletionEmail = Buffer.isBuffer(val) ? val[0] === 1 : Boolean(val);
        }

        if (sendCompletionEmail) {
          const [userRows] = await pool.query("SELECT email, first_name FROM profiles WHERE id = ?", [user_id]);
          if (userRows.length > 0 && userRows[0].email) {
            await sendQuestionnaireCompletionEmail(
              userRows[0].email,
              userRows[0].first_name || "Utente",
              questionnaire_title || "Questionario"
            );
            console.log(`✅ Completion email successfully sent to ${userRows[0].email}`);
          }
        } else {
          console.log("🚫 Completion email blocked by Admin Settings (OFF)");
        }
      } catch (emailErr) {
