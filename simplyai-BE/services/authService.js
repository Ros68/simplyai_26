...
class AuthService {
  ...
  static async getUserById(userId) {
    const [users] = await pool.execute(
      `SELECT p.id, p.email, p.first_name, p.last_name, p.phone, p.address,
            ...
            p.date_of_birth, p.gender, p.job_title, p.vat_number,
            p.city, p.zip_code, p.country,
    ...
    return {
      ...
      // FIX 6.2.4 / 6.2.5 — extended profile fields
      dateOfBirth: user.date_of_birth,
      gender: user.gender,
      jobTitle: user.job_title,
      vatNumber: user.vat_number || user.fiscal_code,
      city: user.city,
  ...
  /**
   * Update user profile — supports all fields including billing/invoice data
   * FIX 6.2.4 / 6.2.5
   */
    ...
    try {
      ...
      const [result] = await pool.execute(
        ...
        [
          firstName, lastName,
          phone || null,
          dateOfBirth || null, gender || null, jobTitle || null,
          vatNumber || null, vatNumber || null,   // keep fiscal_code in sync
          address || null, city || null, zipCode || null, country || "Italia",
          userId,
        ]
      );
